...
import { Search, Bookmark, User, FileText, Briefcase, X } from "lucide-react";

export default function JobSeekerSidebar({ isOpen, setIsOpen }) {
  const navItems = [
  ...
  return (
    <>
      ...
      {/* Sidebar Content */}
      <aside className={`fixed md:sticky top-0 left-0 h-screen w-72 bg-white border-r border-slate-200 flex flex-col shadow-lg md:shadow-sm z-30 transition-transform duration-300 md:translate-x-0 ${isOpen ? "translate-x-0" : "-translate-x-full"}`}>
        <div className="px-6 md:px-8 py-5 md:py-6 border-b border-slate-100 flex items-center justify-between gap-2">
        ...
        <nav className="px-4 py-6 flex-1 space-y-1.5 overflow-y-auto">
          {navItems.map((item) => (
            <NavLink
              key={item.path}
              to={item.path}
              onClick={() => setIsOpen(false)} // Close sidebar on nav link click (mobile)
              className={({ isActive }) =>
